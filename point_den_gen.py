...
file_list = np.loadtxt('Dataset-BRCA-M2C/brca_ds_train.txt', dtype = 'str')

for file in file_list:
    # print(file)
    img = np.array(Image.open('Dataset-BRCA-M2C/images/' + file))
    out_den = np.zeros([int(img.shape[0]/2) + 1, int(img.shape[1]/2) + 1, 3])

    for type in [1,2,3]:
        
        # kde_den = np.zeros( [ int(img.shape[0]/2), int(img.shape[1]/2), 3] )
        labels = np.loadtxt( 'Dataset-BRCA-M2C/labels/' + file.split('.png')[0] + '_gt_class_coords.txt' )
