...
def count_to_class(inpt, c_cc):

    sppl = (815/int(c_cc) * np.arange(1,int(c_cc) + 1)).astype(int)
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pil_copu = pil_copu / 2.2616731919143083e-05
        ...
        out_dict = {}
        out_dict["y"] = np.array(  count_to_class(np.sum(cr_ddd), self.c_num)   , dtype=np.int64)
