...
def count_to_class(inpt):
    sppl = np.array([38.0, 45.0, 51.0, 60.0 ])
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ann_path = os.path.join( self.anndir, self.im_ls[idx].split('.bmp')[0] + '.npy' )

        copu_path = os.path.join( self.copudir, self.im_ls[idx].split('.bmp')[0] + '.npy' )

        ddd_path = os.path.join( self.ddddir , self.im_ls[idx].split('.bmp')[0] + '.npy' )
        ...
        pil_copu = pil_copu/2.0604840985851394e-05

        pil_ddd = np.load(ddd_path)

        cr_im, cr_ann, cr_copu, cr_ddd = random_crop(pil_im, pil_ann,pil_copu,pil_ddd, 256,256)
