...
def count_to_class(inpt):
    sppl = np.array([ 81, 163, 244, 326, 407, 489, 570, 652, 733, 815])
...
class ImageDataset(Dataset):
    def __init__(
        ...
        max_den_val = []
        for idx in np.arange(1,301):
            max_den_val.append( np.max(np.load(self.copudir + '/IMG_' + str(idx) + '.npy'))  )

        self.norm_index = np.max(max_den_val)
    ...
    def __getitem__(self, idx):
        ...
        pil_copu = pil_copu / self.norm_index
